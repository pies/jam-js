 ...
 *
 *  $Id: jam-0.3-dev.js 745 2006-08-02 00:25:58Z Michał $
 */
...
extend('JAM.Lang.Events', {
    ...
    disconnect: function () {
		$A(arguments).each(function(id){
			//debug(id);
			//debug(JAM.Lang.Events._observers);
...
extend('JAM.Dom.Element', {
	...
	off2: function(name) {
		//debug(this.connections.length, this.connections);
		//n = n || (this.connections? this.connections.length: 0);
		for (var i=0; i<this.connections.length; ++i) {
			var id = this.connections[i];//.shift();
			if (id[1]==name) disconnect(this, id);
		}
		return this;
	},
