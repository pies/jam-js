...
define('PACKER_IGNORE', '$1');

class Packer {

	var $parsers = array();

	function Pack ($_SCRIPT, $_ENCODING, $_FAST_DECODE, $_SPECIAL_CHARS) {

		// validate parameters
		$_SCRIPT .= "\n";
		$_ENCODING .= min(1*$_ENCODING, 95);

	}

    // apply all parsing routines
    function _pack($script) {
        for ($i = 0; ($parse = @$this->_parsers[$i]); $i++) {
            $script = call($this, $parse, array($script));
        }
        return $script;
	}
